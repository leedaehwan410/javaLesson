...
public class selectDMLTest {
    ...
    public static void main(String[] args) {
        ...
        //              그 데이터는 콘솔 출력이 되거나 브라우저에서 화면 출력 또는 다른 목적으로 사용될수 있습니다.
    ...
    public static List<CustomerVo> selectAll_4() {
        //4.레코드 행1개를 자바 객체와 1:1로 매핑하기 -> 레코드 행이 많다면 List 에 저장하기 
    ...
    public static void selectAll_2() {
            //2. rs.next() 를 반복해서 레코드 끝가지 위치 이동 확인하기
        Connection connection = OracleConnectionUtil.getConnection();
        String sql = "select * from tbl_custom";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ResultSet rs = pstmt.executeQuery();
            ...
            while (rs.next()) {     //읽어올 다음 레코드가 있으면~ TRUE
                ...
                System.out.println("총"+count+"개의 레코드 행을 조회 햇습니다.");
        } catch (SQLException e) {
