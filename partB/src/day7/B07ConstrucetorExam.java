...
public class B07ConstrucetorExam {
     ...
     */
        ...
        String temp = "Hello~";
        //문자열 객체 생성하기
        //1) 메소드 오버로딩 : 메소드 이름이 같으면서 리턴타입과 인자의 개수와 형식이 다른 정의
        String str1 = new String();                                 //생성자 인자가없음
        ...
        System.out.println("\tstr1 : "+ str1);      //  길이가 0 인 문자열 객체
        System.out.println("\tstr2 : "+ str2);      //
        System.out.println("\tstr3 : "+ str3);      //

        //2) 문자열 리터럴 특징 : 문자열 내용이 같으면 또 메모리에 새로 저장하지 않습니다.
        //                  message 가 참조하는 "Hello~" 가 동일한 리터럴을 temp가 참조할 때,
        ...
        System.out.println("그래서 nextInt 또는 nextDoulbe 대신에 다음 방법으로 하기~~~~~~~~~~~");
        System.out.println("정수 입력>>>");
        temp = sc.nextLine();                   //숫자만 입력하세요.
        num = Integer.parseInt(temp);           //문자열을 int 로 변환하는 메소드 사용.
        System.out.println("실수 입력 >>>");
        temp = sc.nextLine();                   //숫자와 . 만 입력하세요.
        pi = Double.parseDouble(temp);          //문자열을 double 로 변환하는 메소드 사용.
